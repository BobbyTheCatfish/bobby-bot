        ...
        if(read && !(!localTag && msg.client.commands.has(read.name))){
            ...
            if(replaceContent){
                ...
                if(regex.test(replaceContent)){
                    let replace = (str) => u.rand(str.replace(regex, '$1').split('|'))
...
.addCommand({name: 'tag',
    ...
    process: async(msg, args) =>{
        ...
        let cmd = args.toLowerCase().split(' ')[0]
        if(!await Module.db.tags.globalStatus(msg.guild.id) && ['tag','tags'].includes(cmd)) return msg.reply("You can't replace the tag command.").then(u.clean)
        if(!args.split(' ')[1] && msg.attachments.size == 0){
            if(await Module.db.tags.getTag(msg.guildId, cmd)){
                await Module.db.tags.removeTag(msg.guildId, cmd)
        ...
        await Module.db.tags.saveTag(msg.guildId, cmd, args.split(' ').slice(1).join(' ') , msg.attachments.first() ? msg.attachments.first().url : null)
    ...
    process: async(msg, args)=>{
        ...
        if(args.toLowerCase() == 'global' && ((msg.member && msg.member.permissions.has('ADMINISTRATOR')|| msg.author.id == Module.config.ownerId))){
            if(!await Module.db.tags.globalStatus(msg.guild.id)){
                let promptEmbed = u.embed().setTitle('Are you sure you want to go global?').setDescription("You'll still have access to all your current tags, but other servers will have them too")
                ...
                let cancelEmbed = u.embed().setTitle("Global tags left disabled")
            ...
            let gtags = await Module.db.tags.getAllGlobalTags(msg.guild.id)
