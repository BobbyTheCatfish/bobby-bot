    ...
    process: async(msg, args) =>{
        ...
        let prompt = async () =>{
            ...
            msg.author.send({embed}).then(async m =>{
                await m.channel.awaitMessages({filter: emojiFilter, max: 1, time, errors: ['time']}).then(async collected =>{
                    ...
                    let prompt2 = async ()=>{
                        ...
                        msg.author.send({embeds: [embed]}).then(async ms =>{
...
.addEvent("messageReactionAdd", async (reaction, user) =>{
    try {
      ...
      if(dbLookup[0]){
        let member = reaction.message.guild.members.cache.get(user),
        ...
        else if(!reaction.message.guild.roles.cache.get(role)) return (await u.errorChannel(reaction.message)).send({embeds: [u.embed().setTitle('Reaction Role Error').setDescription(`Looks like one of the roles on the reaction role message is no longer around!`)]})
...
.addEvent("messageReactionRemove", async (reaction, user) => {
    try{
        ...
        if(dbLookup){
            ...
            if(!reaction.message.guild.roles.cache.get(role)) (await u.errorChannel(reaction.message)).send({embeds: [u.embed().setTitle('Reaction Role Error').setDescription(`Looks like one of the roles on the reaction role message is no longer around!`)]})
