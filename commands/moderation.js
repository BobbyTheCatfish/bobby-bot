    ...
    process: async (msg, suffix) =>{
        ...
        if(msg.mentions.members.size == 1)
        {
            ...
            if(decision == true)
            {
                ...
                return console.log(`🔔 ${u.time()}${target.user.username.yellow} was banned by ${msg.author.username.red} for ${reason.cyan}`)
            }
            return
        }
...
.addCommand({name: "clear",
    ...
    process: async (msg, suffix) =>{
        ...
        if(isNaN(deleteCount) || !deleteCount || deleteCount < 2 || deleteCount > 100) return msg.channel.send("Please provide a number between 2 and 100 for the number of messages to delete")
        else msg.channel.bulkDelete(deleteCount).catch(error => msg.reply(`Couldn't delete messages because of: ${error}`));
...
.addCommand({name: "dcall",
    ...
    process: async (msg, suffix) =>{
        ...
        for(let m of msg.guild.members.cache){
            ...
            console.log(`! ${u.time()}${msg.member.displayName.yellow} called a DC All in ${msg.guild.nameAcronym.red}, causing ${total.length.green} people to DC`)
            logChannel.send(`${msg.member.displayName} called a DC All in ${msg.guild.nameAcronym}, causing ${total.length} people to DC`)
        };

    }
})
.addCommand({name: 'emoji',
    description: 'Tool for managing server emojis (currently broken)',
    category: 'Mod',
    ...
    process: async (msg, suffix) =>{
        ...
        if(words[0].toLowerCase() == 'add' || words[0].toLowerCase() == 'create'){
            if (msg.attachments.size > 0){
                ...
                if(!words[1] || words[1].length < 2) return msg.channel.send(validName)
                return msg.guild.emojis.create(attachment.url, words[1]).then(e => msg.channel.send({embed: u.embed().setImage(e.url)})).then(msg.channel.send('`:' + words[1] + ':` was successfuly added!')).catch(error => msg.reply(`Couldn't create emoji because of: ${error}`));
            }
            else if(msg.content.endsWith('.png') || msg.content.endsWith('.gif'))
            {
                ...
                return msg.guild.emojis.create(words[1], words[2]).then(msg.channel.send('`:' + words[1] + ':` was successfuly added!')).catch(error => msg.reply(`Couldn't create emoji because of: ${error}`));                
            }
        }
...
.addCommand({name:'lockall',
    ...
    process: async(msg, suffix) =>{
        ...
        {
            channel.updateOverwrite(channel.guild.roles.everyone, {CONNECT: false})// [{ SPEAK: false },{STREAM: false}])
            await msg.react('🔒')
            console.log(`${'C '.cyan}${u.time()} ${(msg.member.displayName).yellow} ${'locked all the VCs!'.green}`)
        }
        else
        {
            channel.updateOverwrite(channel.guild.roles.everyone, {CONNECT: null})//[{ SPEAK: null },{STREAM: null}])
            await msg.react('🔓')
            console.log(`${'C '.cyan}${u.time()} ${(msg.member.displayName).yellow} ${`unlocked all the VCs!`.red}`)
        }
}
})
.addCommand({name:'mute',
    ...
    process: async(msg, suffix) =>{
        ...
        if(!mutedUser) return msg.channel.send(`You need to specify who to mute`)
        ...
        msg.mentions.members.forEach(m => {
            if(m.roles.cache.has(muteRole.id)) f.push(`<@${m.user.id}>`)
            else try{
...
.addCommand({name:'unmute',
    ...
    process: async(msg, suffix) =>{
        ...
        if(f.length > 0) embed.addFields({name: "Failed to unmute", value: f.join('\n')})
...
.addCommand({name:'unmuteall',
    ...
    category: 'Mod',
    permissions: (msg) => (msg.member && msg.member.hasPermission('MUTE_MEMBERS')),
    process: async(msg, suffix) =>{
        let channel = msg.member.voice.channel;
...
.addCommand({name:'nick',
    ...
    category: 'Mod',
    permissions: (msg) => (msg.member && msg.member.hasPermission('MANAGE_NICKNAMES')),
    process: async(msg, suffix) =>{
...
.addCommand({name:'say',
    ...
    category: 'Mod',
    process: async(msg, suffix) =>{
        if(msg.author.id == '337713155801350146' || (msg.member && msg.member.hasPermission("ADMINISTRATOR")))
        {
...
.addCommand({name:'prefix',
    ...
    process: async(msg, suffix) =>{
        ...
        if(!suffix) return msg.channel.send(`The prefix is \`${read}\``).then(u.clean).catch(console.error);
        ...
        return msg.channel.send(`Changed the prefix to \`${read}\``).then(u.clean).catch(console.error);
    }
})
.addEvent('messageReactionAdd', async (reaction, member) =>{
    try {
      if(['📌','📍','🧷'].includes(reaction.emoji.name) && reaction.message.guild.member(member).hasPermission('MANAGE_MESSAGES') && reaction.message.pinnable) await reaction.message.pin()
    } catch (error) {
