        ...
        let sendEmbed = async(msg, embed) =>{
            ...
            msg.author.send({embeds: [promptEmbed]}).then(async(m)=>{
                let resend = async(msg, embed)=>{
                    ...
                    .then(async collected =>{
                        let content = collected.first().content
                        let channel = await msg.guild.channels.cache.filter(b => b.type != 'GUILD_CATEGORY').find(c => c.name.toLowerCase() == content.toLowerCase().replace(/ /g, '-').replace(/#/g, '') || c.id == content)
                        if(!channel){                    
                            ...
                            author.send({embeds: [promptEmbed]}).then(async ms=>{
                                let resend2 = async(msg, embed)=>{
                                    await ms.channel.awaitMessages({filter: contentFilter, max: 1, time, errors: ['time']})
                                    .then(async coll => {
                                        let content = coll.first().content
                                        channel = msg.guild.channels.cache.filter(b => b.type != 'GUILD_CATEGORY').find(c => c.rawPosition == content.replace(/^[0-9]/g))
                                        if(!channel){
