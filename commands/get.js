...
Module.addCommand({ name: "get",
  ...
  process: async (msg, suffix) => {
    const words = suffix.split(' ')[0];
    const keywords = suffix.split(' ').slice(1).join(' ');
    if (['members', 'member', 'user', 'users'].includes(msg.content.toLowerCase())) {
      if (keywords) {
        const object = msg.mentions.members.first() || (await msg.guild.members.fetch({ query: keywords })).first() || msg.member;
        const embed = u.embed();
        const a = object.presence?.activities;
        embed.setTitle(`${object.displayName || 'Unknown'} (${object.user.tag || ''})`)
                ...
                .addFields([
                  { name: 'Roles', value: object.roles.cache.map(r => r).sort((d, e) => e.rawPosition - d.rawPosition).join("\n") || 'None', inline: true },
        ...
        const object = msg.guild.members.cache;
        const bots = object.filter(o => o.user.bot).size;
        const embed = u.embed().setTitle(`There are \`${msg.guild.memberCount}\` members in your server, \`${bots}\` of which are bots`)
                .addFields([
      ...
      if (keywords) {
        const object = msg.guild.roles.cache.find(r => r.toString() == keywords || r.name.toLowerCase() == keywords.toLowerCase() || r.id == keywords);
        ...
        const embed = u.embed().setTitle(object.name).setColor(object.color || null)
                    .addFields([
                      { name: 'ID', value: object.id || 'Unknown', inline: true },
        ...
        const object = msg.guild.roles.cache.map(r => r).join("\n");
        const embed = u.embed().setTitle(`There are \`${msg.guild.roles.cache.size}\` roles in your server.`).setDescription(object);
      ...
      if (keywords) {
        const object = msg.client.emojis.cache.find(e => `<${e.animated ? 'a' : ''}:${e.name}:${e.id}>` == keywords.replace(/ /g, '') || e.name.toLowerCase().replace(/~ /g, '') == keywords.toLowerCase().replace(/~ /g, '') || e.id == keywords);
        ...
        const object = msg.guild.emojis.cache;
      ...
      if (keywords) {
        const object = msg.guild.channels.cache.filter(b => b.type != 'GUILD_CATEGORY').find(c => c.name.toLowerCase() == keywords.toLowerCase() || c.id == keywords);
        if (!object) return msg.channel.send("I couldn't find that channel.");
        const embed = u.embed().setTitle(`#${object.name}`)
                    .addFields([
        ...
        const object = msg.guild.channels.cache.filter(b => b.type != 'GUILD_CATEGORY').filter(b => b.isText()).map(c => c).sort((a, d) => a.rawPosition - d.rawPosition).join(`\n`);
        let vc = msg.guild.channels.cache.filter(b => b.isVoice()).map(c => c).sort((a, d) => a.rawPosition - d.rawPosition).join(`\n`);
        if (vc.length == 0) vc = ' ';
        const embed = u.embed().setTitle(`There are \`${msg.guild.channels.cache.filter(b => b.type != 'GUILD_CATEGORY').size}\` channels in your server.`).setDescription(`${object} \n${vc}`);
      ...
      if (keywords) {
        ...
        const textChildren = object.children.filter(b => b.isText()).map(c => c).sort((a, d) => a.rawPosition - d.rawPosition).join('\n');
        const voiceChildren = object.children.filter(b => b.isVoice()).map(c => c).sort((a, d) => a.rawPosition - d.rawPosition).join('\n');
        const embed = u.embed().setTitle(`${object.name}`)
                .addFields([
                  { name: 'ID', value: object.id, inline: true },
                  { name: 'Created at', value: toEpoch(object.createdTimestamp) || 'Unknown', inline: true },
        ...
        const object = msg.guild.channels.cache.filter(b => b.type == 'GUILD_CATEGORY').sort((a, d) => d.rawPosition - a.rawPosition).map(c => c.name).reverse().join(`\n`);
        const embed = u.embed().setTitle(`There are \`${msg.guild.channels.cache.filter(b => b.type == 'GUILD_CATEGORY').values().length}\` categories in your server`).setDescription(`\`\`\`${object}\`\`\``);
      ...
      const get = await msg.guild.bans.fetch();
      if (keywords) {
        const object = get.find(b => b.user.username.toLowerCase() == keywords || b.user.id == keywords || b.user.tag.toLowerCase() == keywords.toLowerCase());
        ...
        const embed = u.embed().setTitle(`There ${plural ? 'is' : 'are'} \`${get.size}\` banned user${plural ? '' : 's'} in your server`).setDescription(object);
